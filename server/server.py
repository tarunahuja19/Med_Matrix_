...
Usage:
    pip install flask pyngrok jax jaxlib flax numpy
...
app = Flask(__name__)
# Enable CORS for all routes and explicitly allow our custom headers
CORS(app, resources={r"/*": {"origins": "*"}}, allow_headers=["ngrok-skip-browser-warning", "X-API-Key", "Content-Type", "Accept"])
...
def predict_endpoint():
    ...
    try:
        ...
        # ── Return JSON directly for frontend consumption ──────────
...
def predict_download_endpoint():
    """Same as /predict but returns JSON as downloadable file."""
    try:
        expected_key = app.config.get("API_KEY")
        if expected_key:
            provided_key = request.headers.get("X-API-Key") or request.form.get("api_key")
        ...
        if X_complex.ndim == 2:
            X_complex = X_complex[np.newaxis, ...]
        if not np.iscomplexobj(X_complex):
            return jsonify({"error": "Array must be complex-valued."}), 400
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name="predictions.json"
        )
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def main():
    ...
    parser.add_argument("--model_path", type=str,
                        default="best_mri_model (2).msgpack",
